...
def sample_file(tmp_path):
    """Create a small sample binary file with a dummy FMT and data message."""
    path = tmp_path / "test_log.bin"
    with open(path, "wb") as f:
        # Add dummy FMT header
...
def parser(sample_file):
    return MAVParserThreads(sample_file)
...
def test_process_chunk(parser, sample_file):
    ...
    index, messages = parser._process_chunk(args)
    assert index == 0
